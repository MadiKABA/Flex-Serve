...
import { confirmHeroImage, deleteMediaItem, updateMediaMeta } from '@/app/admin/(dashboard)/pages/actions';
...
const SLOT_LABELS = ['Image 1', 'Image 2', 'Image 3'] as const;
...
export default function HeroThreeImages({
    sectionId,
    dbSlug,
    uploadFolder,
    media,
    ...
    const [pending, setPending] = useState<Partial<Record<0 | 1 | 2, PendingSlot>>>({});
    const [busySlot, setBusySlot] = useState<number | null>(null);

    const byPosition = (position: number) => media.find((m) => m.position === position);

    const handleSelect = async (position: 0 | 1 | 2, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const cancelPending = (position: 0 | 1 | 2) => {
        setPending((prev) => {
    ...
    const confirmSlot = async (position: 0 | 1 | 2) => {
        ...
        try {
            ...
            const result = await confirmHeroImage({ sectionId, dbSlug, position, publicId, url });
                    ...
                    <div key={position} className="space-y-1.5">
                        ...
                        {existing && !pendingSlot && (
                            <AltTextField
                                ...
                                onSave={(alt_text) =>
                                    updateMediaMeta(existing.id, dbSlug, { alt_text, title: existing.title ?? '' })
                                }
                            />
                        )}
                        ...
                        {!existing && !pendingSlot && (
                            <button
                                ...
                                className="flex aspect-square w-full flex-col items-center justify-center gap-2 rounded-lg border-2 border-dashed border-[#2E4A6F]/25 text-[#2E4A6F]/60 transition-colors hover:border-[#2E4A6F]/50 hover:text-[#2E4A6F]"
                            >
                        ...
                        {pendingSlot && (
                            <div className="relative aspect-square overflow-hidden rounded-lg border-2 border-dashed border-[#2E4A6F]/40 bg-[#2E4A6F]/5">
